...
class TimeSeriesRAG:
    ...
    def __init__(self, embedding_dim: int = 780):
        ...
        self.index_extra = faiss.IndexFlatIP(embedding_dim)
    ...
    def add_document(self, doc: TimeSeriesDocument) -> None:
        ...
        self.index_extra.add(doc.embedding.reshape(1, -1))
    ...
    def search(self, query_embedding: np.ndarray, k: int = 20) -> List[Dict[str, Any]]:
        ...
        distances_extra, indices_extra = self.index_extra.search(
            query_embedding.reshape(1, -1), k
        )

        for i, idx in enumerate(indices_extra[0]):
            ...
            if idx < len(self.documents):
                ...
                results.append(
                    {
                        "id": doc.id,
                        "distance": float(distances_extra[0][i]),
                        "data": doc.data.tolist(),
                        "metadata": doc.metadata,
                    }
                )

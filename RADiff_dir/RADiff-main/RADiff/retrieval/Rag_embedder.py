...
class TimeSeriesEmbedder:
    ...
    def embed(self, time_series: Union[List[float], np.ndarray]) -> np.ndarray:
        ...
        features = np.concatenate([resampled.flatten(), mean, std, max_val, min_val])

        return features.reshape(1, -1)

    def embed_batch(self, time_series: Union[List[float], np.ndarray]) -> np.ndarray:

        new_time_series = []

        for i in range(time_series.shape[0]):

            resampled = resample(time_series[i], self.target_length)

            mean = np.mean(time_series[i], axis=0)

            std = np.std(time_series[i], axis=0)

            max_val = np.max(time_series[i], axis=0)

            min_val = np.min(time_series[i], axis=0)

            features = np.concatenate(
                [resampled.flatten(), mean, std, max_val, min_val]
            )

            features.reshape(1, -1)

            new_time_series.append(features)
